...
import { Plus, Trash2, Edit2, Image as ImageIcon, Loader2, Star, MapPin, Calendar, X, Check } from 'lucide-react'
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
        <div>
          <h1 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: '38px', color: '#111111', textTransform: 'uppercase', margin: '0 0 4px' }}>Projects</h1>
      ...
      {loading ? <AdminPageLoader label="Loading projects..." /> : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '1.5rem' }}>
          {projects.map(project => (
            <div key={project.id} style={{ background: '#FFFFFF', borderRadius: '12px', border: '1px solid #EBEBEB', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column' }}>
              <div style={{ position: 'relative' }}>
                {project.image_url ? (
                  <img src={project.image_url} alt={project.title} style={{ width: '100%', height: '200px', objectFit: 'cover' }} />
                ) : (
                  <div style={{ width: '100%', height: '200px', background: 'linear-gradient(135deg, #F0F0F0, #E5E5E5)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#CCC' }}>
                    <ImageIcon size={40} />
                  </div>
                )}
                {project.is_featured && (
                ...
                <div style={{ position: 'absolute', top: '12px', right: '12px', display: 'flex', gap: '6px' }}>
                  <button onClick={() => openEdit(project)} style={{ width: '32px', height: '32px', background: '#FFFFFF', border: 'none', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
                    <Edit2 size={14} color="#111" />
                  </button>
                  <button onClick={() => handleDelete(project.id)} style={{ width: '32px', height: '32px', background: '#FFFFFF', border: 'none', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
              ...
              <div style={{ padding: '1.25rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', gap: '6px', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
                  <span style={{ background: '#F7F4EE', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 700, color: '#888', fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '0.06em' }}>{project.category}</span>
                  {project.year && <span style={{ display: 'flex', alignItems: 'center', gap: '4px', background: '#F0F0F0', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', color: '#888' }}><Calendar size={10} />{project.year}</span>}
                  {project.location && <span style={{ display: 'flex', alignItems: 'center', gap: '4px', background: '#F0F0F0', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', color: '#888' }}><MapPin size={10} />{project.location}</span>}
                </div>
                <h3 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: '20px', margin: '0 0 4px', color: '#111' }}>{project.title}</h3>
                ...
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '13px', color: '#888', margin: '0 0 1rem', lineHeight: 1.5, flex: 1, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{project.description}</p>
                <button
                  onClick={() => toggleFeatured(project)}
                  style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 14px', background: project.is_featured ? '#FFF8EC' : '#F5F5F5', color: project.is_featured ? '#A06010' : '#888', border: `1px solid ${project.is_featured ? '#F5D9A0' : '#E5E5E5'}`, borderRadius: '6px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", fontSize: '12px', fontWeight: 600 }}
                >
      ...
      {isOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#FFFFFF', borderRadius: '16px', width: '100%', maxWidth: '640px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.25)' }}>
            <div style={{ padding: '2rem', borderBottom: '1px solid #F0F0F0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: '#FFFFFF', zIndex: 1 }}>
              <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: '24px', margin: 0 }}>{editingId ? 'EDIT PROJECT' : 'NEW PROJECT'}</h2>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                ...
                <div style={{ gridColumn: '1/-1' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                    ...
                    <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '14px', fontWeight: 600 }}>Mark as Featured Project (shown on homepage)</span>
                  </label>
                </div>
              </div>
