...
export default async function BlogIndexPage() {
  ...
  return (
    <main style={{ background: '#F2EFE8', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, paddingTop: '120px', paddingBottom: '7rem', maxWidth: '1280px', margin: '0 auto', width: '100%', paddingLeft: '2rem', paddingRight: '2rem' }}>
        <AnimatedSection>
          <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
            ...
            <h1 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: 'clamp(42px, 5vw, 64px)', lineHeight: 1.05, color: '#111111', textTransform: 'uppercase', margin: 0 }}>
              ...
              <Link href={`/blog/${featuredBlog.id}`} style={{ textDecoration: 'none' }}>
                <div style={{ 
                  ...
                  <div style={{ padding: '4rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 800, fontSize: 'clamp(32px, 3vw, 42px)', color: '#111111', margin: '0 0 1.5rem', lineHeight: 1.1, textTransform: 'uppercase' }}>
                      {featuredBlog.title}
                    </h2>
                    <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '15px', color: '#555', lineHeight: 1.7, margin: '0 0 2rem' }}>
                      {featuredBlog.subheading}
                    </p>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid #F0F0F0', paddingTop: '2rem' }}>
                  ...
                  {featuredBlog.image_url ? (
                    <img src={featuredBlog.image_url} alt={featuredBlog.title} style={{ width: '100%', height: '100%', minHeight: '400px', objectFit: 'cover' }} />
                  ) : (
                    <div style={{ width: '100%', height: '100%', minHeight: '400px', background: '#E5E5E5' }} />
                  )}
                </div>
              </Link>
            </AnimatedSection>
                    ...
                    <Link href={`/blog/${blog.id}`} style={{ textDecoration: 'none', display: 'block', height: '100%' }}>
                      <div className="blog-card" style={{ background: '#FFFFFF', borderRadius: '12px', overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column', boxShadow: '0 4px 20px rgba(0,0,0,0.03)', transition: 'transform 0.3s ease, box-shadow 0.3s ease' }}>
                        {blog.image_url ? (
                          ...
                          <div style={{ width: '100%', height: '240px', background: '#E5E5E5' }} />
                        )}
                        <div style={{ padding: '2rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                          <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '12px', color: '#999', marginBottom: '1rem', fontWeight: 500 }}>
                            {format(new Date(blog.published_at), 'MMMM dd, yyyy')}
                          </div>
                          <h3 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700, fontSize: '24px', color: '#111111', margin: '0 0 1rem', lineHeight: 1.2 }}>
                            {blog.title}
                          </h3>
